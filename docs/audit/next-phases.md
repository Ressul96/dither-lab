...
**Tarih:** 2026-05-23 (M.1 + M.2 + M.4 phase 1+2 + V.1 sonrası güncellendi)
...
**2026-05-23 güncellemesi:** Claude + Codex paralel iki-agent oturumunda **M.1, M.2, M.4 (phase 1 + phase 2) ve V.1** kapandı:
...
- **M.4 phase 1** 13 non-player `innerHTML` site `setInnerHtml` helper'a geçti (commit `812dc68`); helper `Range.createContextualFragment` ile namespace-aware. **Phase 2** graph-render per-node diff + inspector skip-when-unchanged (commit `b1e4767`, `33b89b0`).
...
**Kalan açık işler:** S.1 EXR scope kararı + F22 backlog kalan item'ları (Codex devam ediyor: splash, timeline minimise + default pan ✅; kalan: slider redesign, group in/out, file drag-drop).

**Toplam kalan iş:** ~1 hafta + EXR scope kararına göre +1 hafta.
...
| **F22** | UI polish kalanı (slider / group in-out / file drop) | UI polish | 2-3 gün | Düşük | — |
...
| **M.4** | `innerHTML` → `replaceChildren` (phase 1 + 2) | Tamamlandı 2026-05-23 — `812dc68`, `b1e4767`, `33b89b0` |
...
| **F22 (kısmi)** | Timeline minimise + default pan | Tamamlandı 2026-05-23 — Codex `6018fc5`, `bc14451` |
...
- **Bonus task (Codex devir):** player tier'da 10 innerHTML site (player.js: 6, player-timeline-chrome.js: 2, player-more-menu.js: 1, player-bezier-popover.js: 1) helper'a geçirilebilir. Helper export edili.
...
**Durum:** Devam ediyor — Codex 2026-05-23 oturumunda splash + timeline minimise (`6018fc5`) + default pan (`bc14451`) attı. Splash mount şu an oturumda unstaged (src/js/ui/splash.js, src/index.html, src/js/main.js, src/js/project.js, src/styles/main.css). Kalan F22 backlog item'ları: slider redesign, group in/out node action, file drag-drop.
...
1. **F22 splash + polish backlog (Codex devam ediyor)** — slider redesign, group in/out node action, file drag-drop.
...
3. **M.4 phase 2 takip:** player tier'da 10 innerHTML site için aynı `setInnerHtml` migration; perf ölçümü (timeline scrub render time <2ms hedefi).
4. **M.4 phase 2 + ölçüm:** Per-edge diff (graph-render.js edges hâlâ full-swap), inspector section bazlı diff (şu an top-level skip), perf instrumentation.
